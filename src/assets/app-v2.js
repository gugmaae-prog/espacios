...
function showDetail(r){
  ...
  $('#detail-body').innerHTML=hero+'<div class="detail-content"><span class="detail-badge">'+esc(r.kind==='initiative'?'Future initiative':r.kind||'record')+'</span><h2>'+esc(recordTitle(r))+'</h2><div class="detail-sub">'+esc(recordSub(r))+'</div><div class="detail-facts">'+facts.join('')+'</div>'+(summary?'<div class="detail-section"><h3>Context</h3><p>'+esc(summary)+'</p></div>':'')+verify+developerIntel+'<div class="detail-actions">'+(primary&&primary!=='#'?'<a href="'+esc(primary)+'">Open in PSR</a>':'')+(secondary?'<a class="secondary" href="'+esc(secondary)+'" target="_blank" rel="noopener">Source</a>':'')+'</div></div>';
